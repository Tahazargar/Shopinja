        ...
        <section class="col-12">
            <section class="main-body-container">
                ...
                <section class="d-flex justify-content-between align-items-center mt-4 mb-3 border-bottom pb-2">
                    <a href="{{ route('content.category.index') }}" class="btn btn-info btn-sm">بازگشت</a>
                </section>

                <section>
                    <form action="{{ route('content.category.update', $postCategory->id) }}" id="form" method="POST" enctype="multipart/form-data">
                        @csrf
                        {{ method_field('PUT') }}
                        <section class="row">
                            ...
                            <section class="row my-4">
                                ...
                                <section class="col-md-{{ 6 / $number }}">
                                    <div class="form-check">
                                        <input id="{{ $number }}" type="radio" name="currentImage" value="{{ $key }}" class="form-check-input" @if($key === $postCategory->image['currentImage']) checked @endif>
                                        <label for="{{ $number }}" class="form-check-label mx-2">
                                            <img src="{{ asset($postCategory->image['indexArray'][$postCategory->image['currentImage']]) }}" class="w-100">
                                        </label>
                                    </div>
                                </section>
                                    @php
